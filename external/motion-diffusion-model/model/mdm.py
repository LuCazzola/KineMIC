...
from model.utils import Rotation2xyz
from model.BERT.BERT_encoder import load_bert
from utils.misc import WeightedSum
from argparse import Namespace
from types import SimpleNamespace

#from model.adapters import LoRA, MoE, MultiheadAttentionMoE

from model.adapters import LoRA, MoELoRA

from utils.parser_util import Pfx
#ADAPTER_TYPES = (LoRA, MoE, MultiheadAttentionMoE) # for fast checking of adapter types
ADAPTER_TYPES = (LoRA, MoELoRA)
...
class MDM(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
                 ...
                 arch='trans_enc', emb_trans_dec=False, clip_version=None, stream=None, **kargs):
        super().__init__()
        ...
        print(f'[{stream} stream] Conditioning mode: {self.cond_mode}')
        ...
        assert not self.cond_mode == 'mixed' or self.emb_policy == 'add', 'mixed cond implies add policy'
        ...
        self.target_enc_layers = kargs.get('enc_layers_target', 1)
        ...
        if self.cond_mode != 'no_cond':
            if self.cond_mode in ['text', 'mixed']:
            ...
            if self.cond_mode in ['action', 'mixed']:
    ...
    def clip_encode_text(self, raw_text):
        ...
        max_text_len = 20 if self.dataset in ['humanml', 'kit', 'ntu60', 'ntu120', 'ntu-vibe'] else None  # Specific hardcoding for humanml dataset
    ...
    def plug_adapters(
        ...
        """
        Plug adapters into the model based on the configuration.
        """
        if len(peft) == 0 : # nothing to add
            return

        for adapter_args in peft :
            name = adapter_args.name
            adapter_opt, guidance_opt = Pfx.get_opts_from_adapter_name(name, adapter_args)
            ADAPTER = eval(name)

            attr_map = {
                'transformer_attn' : ['self_attn'] if self.arch == 'trans_enc' else ['self_attn', 'multihead_attn'],
                'transformer_ff' : ['linear1', 'linear2'],
                'conditioning' : ['embed_text'] if self.cond_mode == 'text' else ['embed_action'],
                'input_process' : ['poseEmbedding'],
                'output_process' : ['poseFinal'],
                'timestep_embed' : ['time_embed'],
            }

            target_modules = []
            for module_name in guidance_opt.where :
                target_modules += attr_map[module_name]
            
            ADAPTER.from_module(self, adapter_opt, target_modules=target_modules)
    
    def freeze(self, freeze_list, model_type, stream=None):
        
        def set_grads(module, requires_grad):
            for param in module.parameters():
                param.requires_grad = requires_grad

        # Handle special cases first
        if model_type == 'KineMIC' and stream == 'prior':
            set_grads(self, False)  # Freeze the entire model
            return

        if self.cond_mode == 'mixed':
            # In 'mixed' mode, force text embedder to be frozen and conditioning block to be trainable
            freeze_list.append('embed_text')
            if 'conditioning' in freeze_list:
                freeze_list.remove('conditioning')
        
        # Define a map from component names to model modules
        if self.arch not in ('trans_enc', 'trans_dec'):
            raise ValueError(f'Unsupported arch for freezing: {self.arch}')

        transformer = self.seqTransEncoder if self.arch == 'trans_enc' else self.seqTransDecoder
        attn_attrs = ('self_attn',) if self.arch == 'trans_enc' else ('self_attn', 'multihead_attn')
        
        component_map = {
            'input_process': [self.input_process],
            'output_process': [self.output_process],
            'embed_timestep': [self.embed_timestep],
            'embed_text': [self.embed_text],
            'conditioning': {
                'mixed': [getattr(self, 'embed_text', None), getattr(self, 'embed_action', None)],
                'text': [getattr(self, 'embed_text', None)],
                'action': [getattr(self, 'embed_action', None)],
            }[self.cond_mode],
            'transformer_ff': [getattr(l, a) for l in transformer.layers for a in ('linear1', 'linear2')],
            'transformer_attn': [getattr(l, a) for l in transformer.layers for a in attn_attrs],
        }

        # Freeze components specified in the list
        for name in freeze_list:
            if name == 'norms':
                for module in self.modules():
                    if isinstance(module, nn.LayerNorm):
                        set_grads(module, False)
            elif name in component_map:
                for module in component_map[name]:
                    if module is not None:
                        set_grads(module, False)
        
        print(f"> Froze stream [{stream}] modules: {freeze_list}")
    ...
    def forward(self, x, timesteps, y=None):
        ...
        emb = time_emb # conditioning signal (always include the time embed)
        ...
        if self.cond_mode == 'no_cond': 
            # unconstrained
            pass # time_emb only (already added, that's just a reminder)
        ...
        latent_feats = output.clone()

        output = self.output_process(output)  # [bs, njoints, nfeats, nframes]
        
        return output, {"latent": latent_feats}
