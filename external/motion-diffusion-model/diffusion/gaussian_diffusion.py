...
from data_loaders.unified.scripts import motion_process
from utils.loss_util import masked_lx, masked_goal_lx, LossType, get_geom_loss
from data_loaders.unified.scripts.motion_process import get_target_location
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        **kargs,
        ...
        if self.lambd.rcxyz > 0. or self.lambd.vel > 0. or self.lambd.fc > 0. or self.lambd.target_loc > 0. :
        ...
        self.masked_lx = masked_lx
        self.diff_loss = get_geom_loss(loss_type, prefix='diff')
        self.angle_loss = get_geom_loss(loss_type, prefix='angle')
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        if self.loss_type in [LossType.KL, LossType.RESCALED_KL] :
        ...
        elif self.loss_type in [LossType.MSE, LossType.RESCALED_MSE, LossType.MAE] :
            ...
            # 'Global loss' computed on the full feature vec. (ex. hml_vec, xyz, rot6d, ... it depends on the chosen format)
            terms[f"glob_{self.loss_type.name}"] = self.masked_lx(target, model_output, mask, loss_fn=self.diff_loss)

            target_xyz, model_output_xyz = None, None

            if self.lambd.rcxyz > 0.:
                ...
                terms[f"rcxyz_{self.loss_type.name}"] = self.masked_lx(target_xyz, model_output_xyz, mask, loss_fn=self.diff_loss)  # mean_flat((target_xyz - model_output_xyz) ** 2)
            ...
            if self.lambd.fc > 0.:
                ...
                if self.data_rep == 'rot6d' and dataset.dataname in ['humanact12', 'uestc']:
                    ...
                    pred_vel[~fc_mask] = 0
                    terms["fc"] = self.masked_lx(pred_vel,
                                                 torch.zeros(pred_vel.shape, device=pred_vel.device),
                                                 mask[:, :, :, 1:],
                                                 loss_fn=self.diff_loss)
            if self.lambd.vel > 0.:
                target_vel = (target[..., 1:] - target[..., :-1])
                model_output_vel = (model_output[..., 1:] - model_output[..., :-1])
                terms[f"vel_{self.loss_type.name}"] = self.masked_lx(target_vel[:, :-1, :, :], # Remove last joint, is the root location!
                                                  model_output_vel[:, :-1, :, :],
                                                  mask[:, :, :, 1:],
                                                  loss_fn=self.diff_loss)  # mean_flat((target_vel - model_output_vel) ** 2)
            
            if self.lambd.target_loc > 0.:
                ...
                pred_target = get_target_location(model_output, dataset.mean_gpu, dataset.std_gpu, 
                                            model_kwargs['y']['lengths'], dataset.m_dataset.opt.joints_num, model.all_goal_joint_names, 
                                            model_kwargs['y']['target_joint_names'], model_kwargs['y']['is_heading'])
                terms["target_loc"] = masked_goal_lx(pred_target, ref_target, model_kwargs['y'], model.all_goal_joint_names,
                                                    diff_loss=self.diff_loss, angle_loss= self.angle_loss)
                            

            terms["loss"] = terms[f"glob_{self.loss_type.name}"] + terms.get('vb', 0.) +\
                            (self.lambd.vel * terms.get(f'vel_{self.loss_type.name}', 0.)) +\
                            (self.lambd.rcxyz * terms.get(f'rcxyz_{self.loss_type.name}', 0.)) + \
                            (self.lambd.target_loc * terms.get('target_loc', 0.)) + \
                            (self.lambd.fc * terms.get('fc', 0.))
    ...
    def fc_loss_rot_repr(self, gt_xyz, pred_xyz, mask):
        ...
        # plt.legend()
        # plt.show()
        return self.masked_lx(pred_joint_vel, torch.zeros(pred_joint_vel.shape, device=pred_joint_vel.device),
                              mask[:, :, :, 1:], loss_fn=self.diff_loss)
